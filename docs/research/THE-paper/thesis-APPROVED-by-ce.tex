...
本研究では評価の再現性を重視してWebプラットフォームを採用したが，動的UI（Prism Lattice UI）が最も価値を発揮するのは，画面サイズが限られ，かつユーザーのコンテキスト（場所，時間，移動中など）が頻繁に変化するモバイル環境であると考えられる。
外出先や移動中などのハイコンテキストな状況下では，じっくりと画面操作を行う余裕がないため，
「今の状況に必要なWidgetだけが，適切な設定値で提示される」という動的UIの恩恵は，デスクトップ環境以上に大きなものとなるはずである。
Webでのアーキテクチャ検証を経た次のステップとして，モバイルネイティブアプリへの展開と，センサー情報等を含めたマルチモーダルなコンテキスト認識への拡張は，極めて有望な研究の方向性である。

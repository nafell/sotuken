...
\subsection{データ駆動によるプリミティブ生成とWidget方式の比較}
...
\subsection{固定/動的の切り分け（設計判断の要点）}
\label{sec:design_decision_summary}

設計空間の詳細な検討は\ref{sec:design_space}節で行うが，
本研究のアーキテクチャにおける核となる「固定（Hardened）」と「動的（Generative）」の切り分けをTable~\ref{tab:design_decision_beta21}に示す。
...
\subsection{言語選定：確率的出力に対する検疫レイヤとしてのTypeScript}
...
\subsection{サーバフレームワーク：ヘッドレス検証を可能にするIsomorphic構成}
...
\subsection{ReactiveBindingEngine による依存解決}
...
\subsection{Layer2：運用指標（レイテンシ・推定コスト）}
...
\subsection{VRおよびRC\_SRにおけるボトルネック}
...
\subsection{失敗要因の仮説的整理：仕様境界の問題として}
...
\section{Layer1+指標の導入とその知見}
\label{sec:disc_layer1plus}

\subsection{Layer1+の導入目的}
...
\subsection{観測結果：構成間差異よりも設計意図との乖離の可視化}
...
\subsection{解釈：仕様適合に対するパイプライン設計の支配的影響}
...
\section{RQ2：モデル構成差の影響と運用設計への示唆}
\label{sec:disc_rq2}

\subsection{健全性指標における構成差：構成Bの優位性}
...
\subsection{運用指標における構成差：LATおよびCOSTの有意差}
...
\subsection{モバイル環境への適用とコンテキスト認識の深化}
...
コストは実験ログに記録されたトークン数に、Azure OpenAIの公表単価（2025年12月時点、Table~\ref{tab:token_prices}）を乗じて算出した推定値である。

なお、Azureの実際の請求額は2回の実験の合計で約4,102円であり、実験1（予備実験）終了時点の累計が約2,301円であったことから、実験2のAzure請求額は約1,801円と推定される。
本付録のトークンベース推定値（約1,169円）との差は、model-routerが実際に選択したモデルの単価差、ルーティング手数料（約32円）、Azure側のキャッシュ入力割引等に起因する。
...
Table~\ref{tab:cost_per_stage}にモデル構成別・ステージ別のコスト内訳を示す。
...
\section{Layer1+：仕様適合指標}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{../figures/layer1plus_grouped.png}
...
\section{Layer2：レイテンシとコストのトレードオフ}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../figures/layer2_scatter.png}
\caption{レイテンシとコストの中央値による構成間トレードオフ。構成A・Cは低レイテンシ・高コスト、構成Bは高レイテンシ・低コスト、構成Eは中レイテンシ・最低コストに位置する。}
\label{fig:layer2_scatter}
\end{figure}

\section{Layer2：レイテンシ分布}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../figures/layer2_dist_lat.png}
\caption{モデル構成別のレイテンシ分布（箱ひげ図）。構成Bの中央値が最も高く分散も大きい。構成A・Cは低い中央値と小さな分散を示す。}
\label{fig:layer2_dist_lat}
\end{figure}

\section{Layer2：コスト分布}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../figures/layer2_dist_cost.png}
...
\subsection{REQ\_W2WR\_PRES（W2WR存在整合率）}
...
\subsection{REQ\_BINDING\_COUNT\_OK（Binding数適合率）}
...
\subsection{REQ\_PATTERN\_MATCH（パターン一致率）}
...
\begin{tabular}{lrrrrll}
\toprule
比較 & M1中央値 (ms) & M2中央値 (ms) & $U$ & $p$値 & 有意 & $r$ \\
\midrule

...
設計空間の詳細は\ref{設計空間の整理と本研究のフォーカス}で整理するが，
...
% =========================
% パッチ1（改訂版）: Layer1/Layer4表のデータと説明文
% 挿入箇所: 1265行目の \subsection{結果(集計表)} の直後
% 統計検定結果（statistics-1703.md）に基づく
% =========================
...
なお，再生成率（RGR）においてAll-5-miniのみ88.0\%と他モデルと有意差が見られた（$p<0.001$）が，
これは軽量モデルの出力安定性の課題を示唆している。
...
表\ref{tab:layer4_results_beta21}に，各モデル構成におけるLayer4指標（実用性）の集計結果を示す。
LAT（Latency）は応答時間の中央値（秒），COST（Cost）は1実行あたりのコスト中央値（円），
FR（Failure Rate）はレンダリングエラー発生率を表す。
Hybrid (mini/mini/chat)が最速（7.64秒）かつ最低コスト（3円）を達成し，
All-5-miniは最遅（41.37秒）・最高コスト（4円）であった。
統計検定の結果，レイテンシとコストにおいて多くのモデル間で有意差が確認された（$p<0.001$, Bonferroni補正後）。

\begin{table}[tb]
\caption{実験2：Layer4 実用性（中央値）}
\label{tab:layer4_results_beta21}
\centering
\small
\begin{tabular}{lccc} \toprule
Model & LAT (s) & COST (JPY) & FR \\ \midrule
All-5-Chat & 12.60 & 3 & 33.3\% \\
All-5-mini & 41.37 & 4 & 4.7\% \\
Hybrid (mini/mini/chat) & 7.64 & 3 & 30.0\% \\
Hybrid (chat/mini/mini) & 24.76 & 3 & 6.7\% \\
\bottomrule
\end{tabular}
\end{table}
...
本章では，第\ref{chap:evaluation}章で得られた実験結果をもとに，
提案システム Prism Lattice UI の設計上の含意，モデル構成選択の指針，
および妥当性への脅威と今後の展望を議論する。
本章で扱うモデル構成は，表\ref{tab:model_configs_beta21}に示した A--D の4構成に限定する。

\section{主要発見の要約}

実験2の結果を RQ1/RQ2 に沿って要約する。

\begin{itemize}
  \item \textbf{RQ1（形式健全性の確保）}：
    VR/TCR/RRR（および循環依存の不在を含む構造健全性）が A--D の全構成で 100\% となり，
    \textbf{宣言型 JSON DSL と段階的バリデーションにより，LLM 出力の形式破綻を実用上ほぼ排除できる}
    ことが確認された。
  \item \textbf{RQ2（モデル選択の含意）}：
    形式健全性が天井に達する一方で，\textbf{運用上の指標（LAT/COST）と再生成の必要性（RGR）には構成差が残る}。
    したがってモデル選択は「正しさ」よりも「安定運用（リトライ設計）と効率（待ち時間・コスト）」の最適化問題として現れる。
\end{itemize}

以降では，まず Layer1（構造健全性）の天井効果が示す含意を整理し（\ref{sec:discussion_layer1}節），
次に Layer2（実用性：遅延・コスト）と RGR を踏まえた運用設計上の含意を議論する（\ref{sec:discussion_layer2}節）。
最後に妥当性への脅威と今後の展望を述べる（\ref{sec:discussion_threats}節，\ref{sec:discussion_future}節）。

\section{Layer1（構造健全性）結果の解釈}
\label{sec:discussion_layer1}

\subsection{天井効果：形式健全性はモデル差より「DSL設計＋検証」により支配される}

Layer1 の VR/TCR/RRR（および循環依存の不在）は，A--D の全構成で 100\% を達成した。
この結果は，本研究が採用した「自由度を宣言型 DSL に閉じ込め，機械的に検証可能な不変条件へ落とす」
という設計方針が有効であることを支持する。

特に，LLM を用いた UI 生成では「存在しない識別子参照」「循環依存」「スキーマ逸脱」などが典型的な破綻要因となるが，
本システムではそれらを \textbf{バリデーション（型・参照・循環検査）で検出できる形式}として定義している。
今回の結果は，\textbf{LLM の推論能力に依存せずに，形式的な安全側の下限をシステム側が保証できる}ことを示す。

\subsection{Layer1で差が残るのは「再生成の必要性（RGR）」である}

一方で，Layer1 のペアワイズ検定のうち補正後も有意差が残ったのは RGR（再生成なし率）のみであった。
All-5-mini（B）は RGR=88\% であり，A/C/D（いずれも 100\%）と比較して有意に低い。
これは，最終的な形式健全性（VR等）が 100\% に揃っていても，
\textbf{構成によっては「一発で正しい DSL に着地しにくい」ため，運用上は再生成機構を前提とすべき}ことを示唆する。

言い換えると，本研究の枠組みでは，
\textbf{正しさ（VR等）は「検証と再生成」により回復可能}であるが，
その代償として \textbf{追加の待ち時間やコストが発生しうる}。
この点が Layer2 指標における構成差と結び付く。

\section{Layer2（実用性）と運用設計の含意}
\label{sec:discussion_layer2}

\subsection{レイテンシ：構成差は大きく，ユーザー体験を左右する中心変数となる}

表\ref{tab:layer2_results_beta21}の LAT は構成間で顕著な差が観測された。
中央値ベースでは，C（mini/mini/chat）が最短で，B（All-5-mini）が最長となり，
A（All-5-Chat）と D（chat/mini/mini）はその中間に位置した。
ペアワイズ検定では A--D のいずれの比較でも有意差が確認され，効果量も大きい比較が多い。

この結果は，動的 UI 生成が「ユーザー待ち」を直接発生させる以上，
\textbf{モデル選択は成功率ではなく待ち時間（分布）を主目的に含めて最適化すべき}ことを示す。
また，前節の RGR の結果から，B のように再生成が発生しやすい構成は，
\textbf{見かけのモデル単価とは独立に，エンドツーエンド遅延を悪化させうる}。

\subsection{コスト：単価直感は成り立たず，分布差が効く}

推定コスト（COST）についても構成差が観測された。
中央値は B が 4，A/C/D が 3 であり，B は他構成と比較して有意に高い差が確認された。
さらに，中央値が同一（例：3）であっても，一部の比較では有意差が検出されており，
\textbf{コストの分布（ばらつき，裾の重さ）が運用上の不確実性として残る}ことが分かる。

従って，コスト評価は平均値や中央値のみではなく，
\textbf{上振れ（最悪ケース）を含む運用設計}として議論する必要がある。

\subsection{構成選択の実用的指針：正しさが揃うなら「効率と安定運用」で選ぶ}

本実験条件下では VR/TCR/RRR が全構成で 100\% であるため，
A--D の間で「形式的に正しい UI を生成できるか」という観点での差は観測されなかった。
したがって，実運用での構成選択は次の観点に集約される。

\begin{itemize}
  \item \textbf{応答性最優先}：C のような低レイテンシ構成を優先する。
  \item \textbf{安定運用（リトライ最小化）重視}：RGR が高い構成（A/C/D）を優先し，
        リトライを運用要件として最小化する。
  \item \textbf{総コスト重視}：B のように推定コストが上がりやすい構成は避け，
        価格変動や入力分布変化に対しても頑健な構成を選ぶ。
\end{itemize}

なお，本研究の設計（検証と再生成）により最終的な形式健全性は回復できるため，
\textbf{「再生成のしやすさ」は成功率ではなく運用負荷（遅延・コスト）として現れる}点を明示しておく必要がある。

\section{妥当性への脅威と限界}
\label{sec:discussion_threats}

\subsection{天井効果による識別力の偏り}

本実験では VR/TCR/RRR が全構成で 100\% となった。
これは設計の有効性を示す一方で，Layer1 指標だけではモデル構成差を識別できず，
議論が LAT/COST/RGR といった運用指標に偏る。
今後は，より困難な入力分布（長文，曖昧指示，境界条件）を含め，
形式健全性が真に崩れうる条件での検証が必要である。

\subsection{統計モデルと独立性}

本研究ではログ単位で比較を行ったが，入力テキスト由来の相関が残りうる。
厳密には入力を階層（ランダム効果）として扱うモデルでの再分析余地がある。
ただし本実験では，運用指標において効果量の大きい差が複数観測されており，
結論が完全に覆る可能性は高くないと考える。

\subsection{コスト推定の外的妥当性}

COST はトークン量と単価に基づく推定であり，料金体系や実装（キャッシュ，並列化，ストリーミング等）に依存する。
従って COST の絶対値よりも，相対差と設計上の示唆（再生成が運用負荷へ変換されること）を重視して解釈する必要がある。

\section{今後の展望}
\label{sec:discussion_future}

\subsection{再生成を前提とした運用設計の最適化}

RGR の差が示す通り，軽量モデル中心の構成では再生成が発生しうる。
今後は，再生成回数の上限，打ち切り条件，部分提示 UI，および段階的バリデーションの設計を洗練し，
\textbf{成功率を維持したまま運用負荷（遅延・コスト）を抑える}方策が必要である。

\subsection{意味妥当性とUX評価の拡張}

形式健全性が達成された段階では，研究の焦点は「意味的に適切な UI が生成できるか」
および「Widget-to-Widget Reactivity がユーザー体験を改善するか」に移る。
今後は，意味妥当性（入力意図への整合）と UX 指標（タスク達成・主観評価等）を組み合わせた評価が必要である。

\section{まとめ}

本章では，実験結果をもとに，
(1) 宣言型 DSL と段階的バリデーションにより形式健全性を実用上ほぼ保証できること，
(2) 構成差は主として再生成の必要性（RGR）と運用指標（LAT/COST）として現れること，
(3) よってモデル選択は「正しさ」ではなく「効率と安定運用」で行うべきこと，
を議論した。
これらは，LLM-Hardened DSL に基づく動的 UI 生成システムを実運用へ適用する際の，
実装指針と今後の研究課題を具体化するものである。

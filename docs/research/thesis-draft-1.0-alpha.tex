% =============
% VERSION 1.0-apha
% UPDATE: 3,4,5 根本的に書き直し．重要な設計判断(generatedValue,ports)
% LIMITATION: まだドラフト．実際の仕様と照らし合わせ必要
% =============
...
思考整理支援ドメインに特化した LLM-Hardened DSL システムの実装について述べる。
本実装は，ユーザーが自然言語で現状や悩みを入力する Plan フェーズを対象とし，
その入力内容に応じた動的 UI を生成・表示する。
タスク実行フェーズや長期的な履歴管理は本研究の範囲外とし，
Plan フェーズにおける Stage4 の動的 UI を検証する基盤として実装している。
...
TypeScript は JavaScript に静的型付けを付加した言語であり，
型推論や型アサーション，ユニオン型・ジェネリクスなどの機能を備える。
本研究のように，LLM から受け取る JSON を DSL として取り扱う場合，
\begin{itemize}
  \item 実行時に受け取るデータが不安定になりやすい（フィールド欠落や型不一致など）。
  \item それでもアプリケーション全体を落とさずに，エラー箇所を局所化したい。
\end{itemize}
という要件がある。

TypeScript の静的型検査と漸進的型付けは，この状況に適している。
DSL の型定義を TypeScript の型システム上で表現し，
LLM から受け取った JSON をパースする際に，型ガードや型アサーションを用いて
安全に扱える範囲を明示することで，
\begin{itemize}
  \item 型が保証された部分については，IDE の補完やリファクタリング支援を受けつつ開発できる。
  \item 型が保証できない部分については，ランタイムチェックやフォールバック処理を明示的に書くことで，
        例外の伝搬範囲を制御できる。
\end{itemize}
このように，TypeScript は「LLM 出力の不安定さ」と「アプリケーションとしての堅牢性」の
バランスをとる上で有効な選択となる。
...
\footnote{具体的な件数や例示は付録に掲載するなど，実際の原稿に合わせて調整する。}

\subsection{モデル構成とパラメータ}

実験2で比較するモデル構成は，第\ref{chap:related_work}章で述べた通り，
All-5-Chat，All-5-mini，Hybrid 構成（例：Stage1/2 を mini，Stage3 を Chat など）とする。
温度パラメータは再現性を確保するために $T=0.0$ とし，
その他のパラメータも固定した。
同一の入力テキストに対して，各モデル構成で同じプロンプトテンプレートを用いて実行する。
...
% ここに元の実験2結果の記述を移植し，
% RQ2 と Layer1/Layer2 指標との対応が分かるように整理する。
% 例えば：
% - All-5-Chat は構造健全性と安定性が高いが，LAT/COST が大きい。
% - All-5-mini は安価で高速だが，VR/TCR/RRR が劣る。
% - Hybrid 構成が両者のトレードオフとしてどの程度妥当か。
%
% などを，図表（ヒストグラムや箱ひげ図）とともに説明する。

% （原稿に合わせて記述を追加）

実験2の結果は，第\ref{chap:discussion}章における考察とあわせて，
「どの程度の品質とコストが見込めるなら Stage4 を採用する価値があるか」
という設計判断に対する定量的な根拠を与える。

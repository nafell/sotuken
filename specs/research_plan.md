...
本計画書は、状況適応型の動的UIとプリセット優先スコアリングを用いた「心配事リスト」アプリが、日常の着手行動および不安軽減に与える効果を検証するための研究計画である。実施主体は学術的研究活動であり、再現性と実験容易性を最優先に設計している。

## 1. 研究タイトル
状況適応型・動的UIが不安軽減と着手行動に与える影響：匿名クラウド＋サーバサイドLLM（Gemini 2.5 Flash mini）を用いた実地評価

## 2. 背景・意義
- Todoアプリは優先付け・文脈適合が弱く、ADHD傾向のユーザは起動・着手が困難になりやすい。
- 新規性（Novelty）や「今すぐできる小さな行動」提示が着手を促す可能性があるが、過度な刺激は逆効果になり得る。
- 端末センサ・時間・予定等を用いた「状況適応型UI」と、安定したプリセット重みによる優先付けを組み合わせることで、着手率の向上と不安低下が期待される。

注釈（研究意図）: 背景で示した課題に対し、再現性重視の設計（設定凍結・決定論モード・リプレイ）で効果を検証する。

## 3. 目的
- 主目的: 動的UI（新規性適応）が固定UIと比較して、起動回数・着手率・不安低下を改善するかを検証。
- 副目的: プリセット重みの優先付けのみで行動促進が成立するか、HealthKit 由来の粗区分生体指標が不安低下に寄与するかを評価。

## 4. 研究仮説
- H1: 動的UIは固定UIに比し、起動回数/上位3件着手率/セッション完了率が有意に向上する。
- H2: プリセット重み（個人化なし）でも行動促進効果が成立する。
- H3: 生体情報（心拍/HRVの粗区分）を利用した提案は、直後の自己申告不安低下を増加させる。
...
## 7. インターベンション（アプリ機能）
- 3段階ガイド: 実態の把握→方針決定→行動の分割。
- 優先スコアリング（プリセット）: importance, urgency, relief, deadlineProximity, contextFit, timeFit, staleness, energyMatch, switchCost。
- 状況認識: 時刻/カレンダー/位置カテゴリ/活動/電池/ネット/天気、任意で心拍・HRV（粗区分）。
- 動的UI: サーバ側 LLM（Gemini 2.5 Flash mini、設定・seed固定）でJSON DSL生成。UIはホワイトリスト型コンポーネントのみ。
